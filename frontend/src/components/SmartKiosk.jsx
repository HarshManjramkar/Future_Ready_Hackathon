...
export default function SmartKiosk() {
  ...
  const startRealCamera = async () => {
    setCameraError(null);
    try {
      const html5QrCode = new Html5Qrcode("kiosk-camera-container");
      ...
      // Try raw getUserMedia as fallback
      try {
        ...
        setCameraError("Please allow camera access to use the live attendance kiosk.");
      }
    }
  };
  ...
  // Step 1: Handle ID Card QR Detection
  const handleQrCodeDetected = (rawQrText) => {
    if (step !== 1) return; // Only process when in Step 1

    let clean = rawQrText.replace('{"id":"STU-', '').replace('"}', '').replace('STU-', '').replace('EDU-', '').strip ? rawQrText.strip() : rawQrText;
    clean = clean.split('"')[0].split('-')[0].trim();

    // Match with student record in DB
    const student = students.find(s => 
      String(s.id) === clean || 
      String(s.qr_code) === clean || 
      String(s.student_id) === clean ||
    ...
    // Transition to Step 2: Facial Scrutiny
    setStep(2);
    runFacialScrutiny(clean, targetStudent);
  };

  // Step 2: Run Facial Scrutiny & Liveness Check
  const runFacialScrutiny = (studentId, studentObj) => {
    ...
    const interval = setInterval(() => {
      ...
      if (current >= 100) {
        ...
        submitAttendanceVerification(studentId, studentObj);
      }
    }, 300); // 1.2s total facial scrutiny animation
  };

  // Step 3: Commit Verified Attendance to Backend API
  const submitAttendanceVerification = async (studentId, studentObj) => {
    try {
      ...
      if (data.status === 'SUCCESS') {
        setStep(3); // Step 3: Success Reveal
        confetti({ particleCount: 60, spread: 70, origin: { y: 0.6 } });
        
        // Update local student log
        ...
        // Reset back to Step 1 after 2.5s for next student
        setTimeout(() => {
          resetKioskSteps();
        }, 2800);
      } else {
        // Anti-cheat or invalid ID error
        setStep(1);
        alert(data.message || "Security Alert: Verification failed.");
      }
    } catch (err) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400">2-Step Biometric Anti-Proxy Scanner</span>
          </div>
        ...
        {/* Live Attendance Telemetry Card */}
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 flex items-center gap-6 shrink-0 bg-slate-900/60">
          <div>
            <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Total Enrollment</span>
            <span className="text-lg font-black text-white">{totalCount} Students</span>
          </div>
          <div className="h-8 w-px bg-slate-800" />
          <div>
            <span className="text-[10px] font-mono text-emerald-400 uppercase tracking-wider block">Checked In</span>
            <span className="text-lg font-black text-emerald-400">{presentCount} Present</span>
          </div>
          <div className="h-8 w-px bg-slate-800" />
          <div>
            <span className="text-[10px] font-mono text-sky-400 uppercase tracking-wider block">Attendance Rate</span>
            <span className="text-lg font-black text-sky-400">{attendanceRate}%</span>
          </div>
        </div>
      </div>

      {/* Main Grid: Full-Width 2-Step Camera Viewport + Live Roster Log */}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={`p-3.5 rounded-2xl border transition-all flex items-center gap-3 ${
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400">Hold ID QR code to camera</p>
              </div>
            </div>
            ...
            <div className={`p-3.5 rounded-2xl border transition-all flex items-center gap-3 ${
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400">Biometric liveness check</p>
              </div>
            </div>
          </div>

          {/* Full Screen Camera Module Viewport */}
          <div className={`relative rounded-3xl overflow-hidden aspect-video bg-slate-950 border-2 transition-all shadow-2xl flex items-center justify-center ${
            ...
            {/* Live HTML5 QR Camera Container */}
            <div id="kiosk-camera-container" className="w-full h-full object-cover" />
            ...
            {step === 1 && (
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center p-6 bg-gradient-to-t from-slate-950/80 via-transparent to-slate-950/40">
                <div className="w-64 h-64 border-2 border-dashed border-emerald-400/80 rounded-3xl relative flex items-center justify-center shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                  {/* Laser Scanning Line */}
                ...
                <div className="mt-4 px-4 py-2 rounded-xl bg-slate-900/90 border border-slate-800 text-xs font-mono text-emerald-400 flex items-center gap-2 shadow-lg backdrop-blur-md">
                  <Sparkles className="w-4 h-4 animate-spin text-emerald-400" />
                  <span>STEP 1: Hold Student ID Card up to camera lens...</span>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center p-6 bg-slate-950/60 backdrop-blur-xs">
                <div className="w-56 h-72 border-2 border-sky-400 rounded-3xl relative flex flex-col items-center justify-between p-4 shadow-[0_0_35px_rgba(56,189,248,0.5)] animate-pulse">
                  <span className="bg-sky-500 text-slate-950 font-black text-[10px] px-2.5 py-0.5 rounded-full shadow">
                  ...
                  <div className="text-center space-y-1 my-auto">
                    ...
                    <p className="text-xs font-bold text-white">{matchedStudent?.name || "Student"}</p>
                    <p className="text-[10px] font-mono text-sky-300">ID: STU-{scannedId}</p>
                  </div>
                ...
                <div className="mt-4 px-4 py-2 rounded-xl bg-slate-900/90 border border-sky-500/30 text-xs font-mono text-sky-300 flex items-center gap-2 shadow-lg">
                  <ShieldCheck className="w-4 h-4 text-sky-400 animate-spin" />
                  <span>Analyzing facial liveness biometrics ({scrutinyProgress}%)...</span>
                </div>
              </div>
            )}

            {/* Step 3 HUD: Verified Attendance Success Badge */}
            {step === 3 && matchedStudent && (
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center p-6 bg-emerald-950/80 backdrop-blur-md animate-in fade-in zoom-in duration-300">
                <div className="w-20 h-20 rounded-3xl bg-emerald-500 text-slate-950 flex items-center justify-center shadow-2xl mb-4 animate-bounce">
                  <CheckCircle2 className="w-12 h-12" />
                </div>
                <h3 className="text-2xl font-black text-white tracking-tight">ATTENDANCE VERIFIED!</h3>
                <p className="text-sm font-bold text-emerald-300 mt-1">{matchedStudent.name} ({matchedStudent.grade || "Grade 10-A"})</p>
                <div className="mt-3 px-4 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-xs font-mono text-emerald-300 flex items-center gap-2">
                  <Clock className="w-3.5 h-3.5" />
                  <span>Check-in Recorded: {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              </div>
            )}
          </div>
